...
def get_data(path, double_thr=1e10, small_filter=False):
    """
    ...
    for item in l:
        ...
        if small_filter and newdict['cpu'] < 4:
            continue
...
class Server(object):
    ...
    def alloc_rate(self, numa, cpu_selection):
        """
            calculate allocation rate after creation.
            cpu_alloc_rate if cpu_selection is True.
            mem_alloc_rate if cpu_selection is False.
        """
        if cpu_selection:
...
class Cluster:
    ...
    def best_fit_action(self, request, cpu_selection):
        ...
        for server in self.servers:
            ...
            if request['is_double'] == 1 and usable[0] == 1:
                ...
                alloc_rate = max(server.alloc_rate(0, cpu_selection), server.alloc_rate(1, cpu_selection))
                if alloc_rate < bst_alloc_rate:
                    bst_alloc_rate = alloc_rate
                    bst_action = i * 2
                ...
                for numa in range(2):
                    if usable[numa] == 1:
                        ...
                        alloc_rate = min(bst_alloc_rate, server.alloc_rate(numa, cpu_selection))
                        if alloc_rate < bst_alloc_rate:
                            bst_alloc_rate = alloc_rate
                            bst_action = i * 2 + numa
                        server.delete(request)
        ...
        for server in self.servers:
            ...
            for numa in range(2):
                if usable[numa] == 1:
                    server.handle(request, numa)
                    if request['is_double'] == 1:
                        my_scores.append(max(server.alloc_rate(0, True), server.alloc_rate(1, True)))
                        my_scores.append(max(server.alloc_rate(0, True), server.alloc_rate(1, True)))
                        server.delete(request)
                        break
                    my_scores.append(server.alloc_rate(numa, True))
                    server.delete(request)
                else:
                    my_scores.append(bst_delta)
        if request['is_double'] == 1:
            k *= 2
        avail_indices = sorted(range(len(my_scores)), key=lambda i: my_scores[i])[:k]
        res = np.zeros(len(self.servers) * 2)
        for ind in avail_indices:
            if my_scores[ind] < bst_delta:
                res[ind] = 1
        return res
    ...
    def reset(self):
        ...
        """
        self.servers = [Server(self.cpu, self.mem) for _ in range(self.N)]
    ...
    def cal_cpu_allocation_rate(self):
        ...
        allocation_rate = []
        for server in self.servers:
            allocation_rate.append(1 - server.alloc_rate(0, True))
            allocation_rate.append(1 - server.alloc_rate(1, True))
...
class SchedEnv(gym.Env):
    ...
    def __init__(self, N, cpu, mem, path, render_path=None, allow_release=False, rew_fn='iden', double_thr=1e10,
                 topk=3):
        """
        ...
        self.requests = get_data(path, double_thr)
        self.rew_fn = rew_fn
        self.max_delta = 5000
        self.t = 0
        self.dur = 0
        ...
        while request['type'] == 1:
            if self.allow_release:
                ...
                if self.isrender and del_status != -1:
                    record_dict = {'server': self.cluster.describe(), 'request': self.requests[self.t],
                                   'action': del_status}
